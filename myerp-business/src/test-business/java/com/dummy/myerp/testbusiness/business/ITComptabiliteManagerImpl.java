...
public class ITComptabiliteManagerImpl {
    ...
    */
    ...
    public void insertEcritureTest() throws FunctionalException {
        ...
        //Fournisseur = 401
        ecritureComptable.getListLigneEcriture().add(new LigneEcritureComptable(new CompteComptable(401), null, new BigDecimal(125), null));
        
        //banque = 512
        ecritureComptable.getListLigneEcriture().add(new LigneEcritureComptable(new CompteComptable(512), null, null, new BigDecimal(125)));
    ...
    //********************************************************************************************//
       ...
       */
        ...
        ecritureComptable = sp.filter(o -> o.getReference().equals("AC-2019/00001")).findFirst().get();
        ...
        assertTrue("Vérification que la nouvelle écriture est bien en db", listeEcritureCompta.stream().filter(o -> o.getReference().equals("AC-2019/00002")).findFirst().isPresent());
        assertFalse("Vérification que l'ancienne écriture n'est plus en db", listeEcritureCompta.stream().filter(o -> o.getReference().equals("AC-2019/00001")).findFirst().isPresent());
        
        
        //Nous modifions une écriture éronnée
        sp = listeEcritureCompta.stream();
        
        ecritureComptable = sp.filter(o -> o.getReference().equals("AC-2019/00002")).findFirst().get();
        ecritureComptable.getListLigneEcriture().clear();
        ...
        @Test
         ...
         ecritureComptable.setLibelle("Libelle");
           ...
           this.comptaManager.addReference(ecritureComptable);
            ...
            assertTrue("sequence existante :"+ ecritureComptable.toString(), ecritureComptable.getReference().equals("AC-2019/00041"));
            assertTrue("sequence mise a jour en db sur existante", lastSequence.getAnnee()==2019&&lastSequence.getDerniereValeur()==41);
